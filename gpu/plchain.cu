...
int plchain_finish_batch(streamSetup_t stream_setup, int stream_id, Misc misc, void* km){
    ...
    plchain_debug_analysis_long(stream_setup.streams[stream_id], kernel_throughput);
#endif  // DEBUG_CHECK
#ifdef DEBUG_PRINT
    ...
    float kernel_runtime_ms[MICRO_BATCH + 1];
...
#ifdef DEBUG_PRINT
    fprintf(stderr, "[Info] %s (%s:%d) MICROBATCH$%d FINISHED: long seg %lu - %u", 
